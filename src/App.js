  ...
  useEffect(() => {
    ...
    function writeNumbers(number_of_lines) {
      ...
      for (let i = 0; i < line.numPoints; i++) {
        //y axis
        ctx.fillText(i, canvas.width/2-15, canvas.height/2 + x_zoom * -1 * proportion * i*canvas.height/(number_of_lines*2)+15); //todo zoom
    
        if(i === 0) continue; //only 1 zero
        ctx.fillText(-i, canvas.width/2-15, canvas.height/2 + x_zoom * proportion * i*canvas.height/(number_of_lines*2)+15); //todo zoom

        //x axis
        ctx.fillText(i, canvas.width/2 + x_zoom*i*(canvas.width/(number_of_numbers*2)), canvas.height/2+15);
        ctx.fillText(-1*i, canvas.width/2 - x_zoom*i*(canvas.width/(number_of_numbers*2)), canvas.height/2+15);
      }
    }
